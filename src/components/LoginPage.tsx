...
export function LoginPage({ onLoginSuccess, activeTheme }: LoginPageProps) {
  ...
  const [viewMode, setViewMode] = useState<'login' | 'forgot-password' | 'otp-verification' | 'success' | 'reset-password' | 'two-factor-auth'>('login');
  ...
  const handleTwoFactorSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!twoFactorCode) {
      setTwoFactorError(selectedLanguage === 'de' ? 'Bitte geben Sie einen gültigen Code ein.' : 'The verification code cannot be empty.');
      return;
    }
      ...
      <div className="md:w-1/2 flex flex-col justify-center items-center p-6 md:p-16 relative" id="login-right-pane">
        ...
        <div className="w-full max-w-md bg-transparent p-4 relative" id="login-inner-box">
          <AnimatePresence mode="wait">
            ...
            {/* TWO-FACTOR AUTH SCREEN (Directly mimics the layout & style of the recover password screen) */}
            {viewMode === 'two-factor-auth' && !showFeedback && (
              <motion.div
                key="two-factor-auth-view"
                initial={{ opacity: 0, x: 10 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -10 }}
                transition={{ duration: 0.3 }}
                id="two-factor-auth-view"
                className="w-full"
              >
                <button
                  type="button"
                ...
                <div className="mb-6">
                  <h2 className="text-3xl font-medium tracking-tight text-neutral-900 font-sans" style={{ color: activeTheme.textColor }}>
                    {selectedLanguage === 'de' ? 'Zwei-Faktor-Authentifizierung' : 'Two-factor authentication'}
                  </h2>
                  <p className="text-sm font-medium mt-3" style={{ color: activeTheme.primaryColor }}>
                    {selectedLanguage === 'de' ? 'Bitte geben Sie den Bestätigungscode ein' : 'Please enter the verification code'}
                  </p>
                </div>

                <form onSubmit={handleTwoFactorSubmit} className="space-y-6" id="two-factor-form">
                  {twoFactorError && (
                    <div 
                      className="p-4 rounded-lg border border-red-200 bg-[#FFF0F0] text-red-700 text-xs font-semibold border-l-4 border-l-[#E52222] animate-fadeIn mb-4 font-sans"
                      id="two-factor-error-box"
                    >
                      {twoFactorError}
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder={selectedLanguage === 'de' ? 'Code eingeben' : 'Code'}
                        id="two-factor-code-input"
                      />
                    </div>
                  </div>

                  {/* Verify Action Button styled precisely with XFAIR corporate colors */}
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={isVerifying2FA}
                      className="w-full text-white font-bold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm hover:brightness-105 active:scale-[0.99] disabled:opacity-50 cursor-pointer text-sm font-sans"
                      style={{ backgroundColor: activeTheme.primaryColor }}
                    ...
                    >
                      {isVerifying2FA ? (
                        <span className="flex items-center gap-2">
                          <span className="animate-spin inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full" />
                          {selectedLanguage === 'de' ? 'Wird überprüft...' : 'Verifying...'}
                        </span>
                      ) : (
                        <>
                          <span>{selectedLanguage === 'de' ? 'Weiter' : 'Next'}</span>
                          <ArrowRight className="w-4 h-4" />
                        </>
                      )}
                    </button>
