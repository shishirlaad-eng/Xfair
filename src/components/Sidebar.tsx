...
export function Sidebar({ activePage, onNavigate, activeTheme, onLogout, onAddEmployeeMock }: SidebarProps) {
  ...
  const isStevenSearchActive = useMemo(() => {
    const term = searchName.trim().toLowerCase();
    return term.includes('steven') || term.includes('terry') || term === 'steven';
  }, [searchName]);

  const stevenTerryCategory: SidebarCategory = useMemo(() => ({
    id: 'steven-terry',
    label: 'Steven Terry',
    icon: User,
    items: [
      { id: 'personal-data', label: 'Personal data' },
      { id: 'accompanying-person-single', label: 'Accompanying person (single)' },
      { id: 'accompanying-persons', label: 'Accompanying persons' },
      { id: 'attendance', label: 'Attendance' },
      { id: 'attendance-time', label: 'Attendance / time' },
      { id: 'attendance-month', label: 'Attendance by month' },
      { id: 'attendance-tasks', label: 'Attendance with tasks and functions' },
      { id: 'attendance-timeslots', label: 'Attendance with Timeslots' },
      { id: 'criteria', label: 'Criteria' },
      { id: 'criteria-groups', label: 'Criteria groups' },
      { id: 'data-security', label: 'Data security' },
      { id: 'deputies', label: 'Deputies' },
      { id: 'hotel-request', label: 'Hotel request' },
      { id: 'orders', label: 'Orders' },
      { id: 'personal-calendar', label: 'Personal calendar' },
      { id: 'picture-gallery', label: 'Picture gallery' },
      { id: 'postcode-range', label: 'Postcode range' },
      { id: 'responsible-persons', label: 'Responsible persons' },
      { id: 'services', label: 'Services' },
      { id: 'tasks-functions', label: 'Tasks / functions' },
      { id: 'timeslots-functions', label: 'Time slots / functions' },
      { id: 'travel', label: 'Travel' },
      { id: 'visa-invitation', label: 'Visa invitation' },
      { id: 'mobile-app-pin', label: 'Mobile app pin codes' },
      { id: 'invoices', label: 'Invoices' },
      { id: 'coordinator', label: 'Coordinator' },
      { id: 'responsibility', label: 'Responsibility' },
      { id: 'chat', label: 'Chat' },
      { id: 'message-history', label: 'Message history' },
      { id: 'access-control', label: 'Access control' },
      { id: 'sales-orders-sub', label: 'Sales orders' },
      { id: 'rights-sub', label: 'Rights' },
      { id: 'permissions-sub', label: 'Permissions' },
      { id: 'reporting-access', label: 'Reporting access' },
      { id: 'settings-sub', label: 'Settings' },
  ...
  const filteredCategories = useMemo(() => {
    ...
    if (isStevenSearchActive) {
      return [stevenTerryCategory, ...list];
    }
    return list;
  }, [navSearch, categories, isStevenSearchActive, stevenTerryCategory]);

  // Automatically expand Steven Terry category on search activation
  React.useEffect(() => {
    if (isStevenSearchActive) {
      setExpandedCats(prev => ({
        ...prev,
  ...
  return (
    <>
      <aside 
        ...
        style={{
          backgroundColor: activeTheme.secondaryColor || '#1E1E24', // Modern Obsidian Charcoal Canvas or selected secondary color
                  ...
                  {!isCollapsed && isExpanded && (
                    <div className="pl-6 border-l border-white/5 ml-4 space-y-1.5 py-1">
                      {cat.items.map(item => {
                        ...
                        const isClickable = item.id === 'change-password' || item.id === 'theme-settings' || item.id === 'event-manager-conf' || item.id === 'resource-editor';
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left py-1.5 px-3 rounded-lg text-[11px] font-semibold transition-all block truncate ${
                              isActiveSub && isClickable
                            ...
                            style={{
                              backgroundColor: isActiveSub && isClickable ? activeTheme.primaryColor : undefined
                            }}
                          >
